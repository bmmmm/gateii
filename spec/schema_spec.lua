...
describe("schema.validate_openrouter_free", function()
    ...
    it("accepts positive daily_limit and minute_limit", function()
        local ok = schema.validate_openrouter_free({ daily_limit = 1000, minute_limit = 20 })
        assert.is_true(ok)
    end)

    it("rejects non-positive or non-integer limits", function()
        local ok, err = schema.validate_openrouter_free({ daily_limit = 0 })
        assert.is_false(ok); assert.is_not_nil(err)
        local ok2, err2 = schema.validate_openrouter_free({ minute_limit = "20" })
        assert.is_false(ok2); assert.is_not_nil(err2)
    end)
end)
