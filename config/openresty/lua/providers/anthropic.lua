...
-- Parses inline without building full event table — exits early once both message_start and message_delta found
    ...
    for raw_line in (body .. "\n"):gmatch("([^\n]*)\n") do
        ...
        if line == "" then
            ...
            if #data_lines > 0 or current_event then
                ...
                -- Remove trailing newline per spec
                if data:sub(-1) == "\n" then data = data:sub(1, -2) end
                local event_type = current_event or "message"

                -- Extract tokens immediately instead of collecting all events
                ...
                -- Early exit: once both message_start and message_delta have been found, stop parsing
                if found_start and found_delta then
                    break
                end
            end
            current_event = nil
