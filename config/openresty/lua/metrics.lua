...
local function _bench_to_metrics()
    ...
    add("# HELP gateii_omlx_bench_latency_seconds Local-agent bench median latency per task+model")
...
local function _routing_to_metrics()
    local f = io.open("/etc/nginx/data/agents/routing.json", "r")
    if not f then return end
    local content = f:read("*a"); f:close()
    local r = cjson.decode(content)
    if not r or not r.routes then return end
    add("# HELP gateii_omlx_routing_choice 1.0 = task currently routes to this model (per data/agents/routing.json)")
