#!/bin/bash
# deploy-runner-host — push gateii stack to the NUTC host.
#
# rsync flow:
...
SSH_HOST="${GATEII_SSH_HOST:-runner-host}"
...
ADMIN_TOKEN=stub GRAFANA_ADMIN_PASSWORD=stub \
    docker compose -f servers/runner-host/server/stacks/gateii/docker-compose.yml config \
    >/dev/null 2>&1 \
    || die "compose validation failed — run: ADMIN_TOKEN=x GRAFANA_ADMIN_PASSWORD=x docker compose -f servers/runner-host/server/stacks/gateii/docker-compose.yml config"
...
rsync -az --inplace \
    servers/runner-host/server/stacks/gateii/docker-compose.yml \
    servers/runner-host/server/stacks/gateii/README.md \
    "$SSH_HOST:$REMOTE_DIR/"
