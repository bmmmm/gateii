...
# Runs on the deployment host (e.g. runner-host). Idempotent — safe to re-run.
#
# Usage:
...
if ! git remote get-url origin 2>/dev/null | grep -q "your-org/gateii"; then
  echo "ERROR: origin remote does not point to your-org/gateii" >&2
  exit 1
fi
