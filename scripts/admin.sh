  ...
  bootstrap)
    ...
    case "$ACTION" in
      create)
        ...
        echo ""
        echo -e "${DIM}Option 1: Individual environment variables${NC}"
        echo -e "  export GATEII_URL=$PROXY"
        echo -e "  export GATEII_BOOTSTRAP_CODE=$CODE"
        echo -e "  export GATEII_BOOTSTRAP_SECRET=$SECRET"
        echo -e "  bash scripts/gateii-connect.sh"
        echo ""
        echo -e "${DIM}Option 2: Single connection string${NC}"
        echo -e "  export GATEII_CONNECT=$CODE:$SECRET@$PROXY"
        echo -e "  bash scripts/gateii-connect.sh"
        echo ""
        ;;
      list)
