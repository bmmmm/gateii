...
case "$SUBCMD" in
  ...
  switch)
    TARGET="${1:?Usage: admin.sh switch <local|runner-host|direct>}"
    ...
    case "$TARGET" in
      ...
      runner-host)
        ...
        wait_health "$NUTC" "runner-host" || exit 1
        set_base_url "$NUTC" "runner-host proxy"
        ;;
      direct)
      ...
      status)
        ...
        ;;
      *)
        echo -e "${RED}Unknown target '$TARGET' — use 'local', 'runner-host', 'direct', or 'status'${NC}" >&2; exit 1
        ;;
    esac
    ;;
...
YAML
  ...
  help|--help|-h|"")
    ...
    echo "  switch runner-host                     Route Claude Code through the NUTC proxy (NUTC_URL in .env)"
