...
| `config/openresty/lua/openrouter_free.lua` | Cached loader for `data/openrouter-free.json` (OpenRouter `:free` pool + default) + free-tier budget bookkeeping: per-window request counting, 429-armed exhaustion signal (drives handler's 503), budget snapshot for /metrics + admin GET; `fallback_models()` builds the injected `models` array (skipped when the client sends `x-gateii-no-fallback` — pinned-model evals) |
